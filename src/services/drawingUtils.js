 ...
 */
export const drawDetections = (ctx, detections, modelConfig) => {
 ...
 */
export const drawVideoFrame = (ctx, videoElement) => {
  if (!ctx || !videoElement) {
    return;
  }
 ...
 */
export const drawTrackingPaths = (ctx, trackingPaths, classColorMap) => {
 ...
 * @param {Array} detections - Array of detection results
 */
export const drawDetectionHeatmap = (ctx, detections) => {
  if (!ctx || !detections || !Array.isArray(detections)) {
    return;
  }
